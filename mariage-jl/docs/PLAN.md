...
- **Accessibilité WCAG 2.2 AA** : contrastes mesurés (les 5 couleurs ne servent jamais de couleur de texte), cibles ≥ 48 px, clavier et lecteur d'écran en FR et EN, zoom 200 %, sous-titres pour toute vidéo parlée, `prefers-reduced-motion` respecté.
  **Vérifié automatiquement** (axe-core, étiquettes WCAG 2.0/2.1/2.2 A et AA) sur les sept écrans d'invité : zéro anomalie. La règle `target-size` est remplacée par une mesure plus stricte — chaque cible fait au moins 48 px de haut, contre 24 px exigés — parce qu'axe signale toute cible recouverte par la barre d'onglets, qui est opaque et flotte au-dessus du contenu comme toute barre de navigation mobile. Le gabarit réserve la hauteur de la barre en bas de page pour que la fin de chaque écran reste atteignable.
- **Performance** : Lighthouse mobile ≥ 95 sur les 4 catégories ; première page utile < 2 s en 4G lente ; film jamais préchargé ; polices auto-hébergées avec `font-display: swap`.
  **Mesuré le 17/09/2026** sur l'application compilée (`pnpm lighthouse`, seuil vérifié en intégration continue) : Performance **99**, Accessibilité **100**, Bonnes pratiques **100** sur l'accueil, le programme et la FAQ. Deux corrections ont été nécessaires : les polices n'étaient pas préchargées (LCP de 3,1 s sur la date en Bodoni, ramené sous la seconde en auto-hébergeant les fichiers), et une icône manquante provoquait un 404 en console.

**[ANOMALIE_LOGIQUE : Lighthouse SEO ≥ 95 contre l'absence d'indexation]**
Le brief §12 demande ≥ 95 dans les quatre catégories, dont « SEO ». Or le §11 impose qu'aucune donnée d'invité ne soit indexable : l'application est en `noindex`, ce qui fait échouer l'audit « la page est indexable » et plafonne la catégorie SEO à 60. Les deux exigences sont incompatibles, et c'est la confidentialité qui doit gagner. Le seuil est donc vérifié sur les trois autres catégories, et l'audit `is-crawlable` est explicitement exclu, pas ignoré.
- **Hors ligne** : programme, infos, FAQ, table et fiche de secours consultables après une première visite ; réponses et médias en file d'attente.
  **Livré et vérifié le 17/09/2026.** Ce qui entre en cache : les navigations vers `/programme`, `/infos`, `/faq` et l'écran « Pas de réseau », précaché. Ce qui n'y entre **jamais** : l'accueil, la réponse, le partage, l'espace des mariés, et les charges RSC que Next précharge derrière les liens — elles contenaient le nom du foyer et sa réponse, et faisaient en outre grossir le cache sans fin. Un test vérifie le contenu exact des caches.
  **Méthode** : `context.setOffline()` ne bloque pas la boucle locale sous Chromium — un test bâti dessus ne vérifie rien. Les parcours hors ligne démarrent leur propre serveur et le coupent.
  **Limite iOS, inchangée** : rien ne repart tant que l'application est fermée ; la reprise a lieu à la réouverture, et l'écran le dit. La file rattrape aussi le cas « le téléphone se croit connecté mais rien ne passe », que l'indicateur du navigateur ne signale pas.
...
- **Compatibilité** : Safari iOS et Chrome Android (deux dernières versions majeures), un Android d'entrée de gamme, un iPhone ancien encore maintenu `[modèles de test À COMPLÉTER]`.
  **Limite de l'automatisation** : WebKit n'est pas installé dans l'environnement de développement, les parcours Playwright « iPhone » tournent donc sous Chromium — ils valident la mise en page et les gestes, pas le moteur de Safari. Une passe manuelle sur un iPhone réel est obligatoire avant la validation de V1, et fait partie des tests humains de la section 0 bis.
